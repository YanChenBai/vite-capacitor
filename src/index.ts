...
export function viteCapacitor(): Plugin {
  if (!fs.existsSync(CAP_STORAGE_DIR))
    fs.mkdirSync(CAP_STORAGE_DIR, { recursive: true })

  return {
    name: 'vite-plugin-vite-capacitor',

    configureServer(server) {
      server.httpServer?.on('listening', () => {
        const { config } = server
        const port = config.server.port ?? 5173
        const baseCapConfig = config.capacitor || {}
